  ...
  const navLinks = [
    ...
    { name: "Company", href: "/company"},
    {
      name: "Collections", href: "/collections", children: [
        ...
        { name: "Desktops", href: "/collections/destops" }
      ]
    },
        ...
        <ul className='hidden md:flex'>
          {
            navLinks.map((link) => {
              ...
              if (link.children) {
                ...
                return (
                  ...
                  >
                    <span className="flex items-center cursor-pointer">
                      {link.name} <IoIosArrowDropdown className="ml-1 text-2xl" />
                    </span>
                    {dropdown && (
                ...
                //     onMouseEnter={() => link.name === "Collections" && setDropdown(true)}
                //     onMouseLeave={() => link.name === "Collections" && setDropdown(false)}
                //   >
                //     <Link href={link.href} className={`${isActive ? activeClasses : inactiveClasses}`}>
                //       {link.name} 
                //     </Link>
                //     {link.children && dropdown && (
                //       <ul className="absolute left-0 mt-2 w-96 bg-white text-black shadow-lg">
                //         {link.children.map((child) => (
                //           <li key={child.name} className="hover:bg-gray-100">
                //             <Link href={child.href} className="block px-4 py-2">{child.name}</Link>
                //           </li>
                //         ))}
                //       </ul>
                //     )}
